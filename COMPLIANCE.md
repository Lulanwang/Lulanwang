...
| Treatment-planning artifacts (Round 9) | Regulatory + Clinical | The `/studies/{id}/plan` workspace, `treatment_plans` / `contours` tables, dose-synthesis model, and the constraint evaluator are **research mock-ups only**. The system is not a TPS (treatment planning system). Doses are synthetic Gaussian-superposition illustrations — not Monte Carlo. Plans cannot be exported to a delivery system. Production RT use requires a 510(k)-cleared TPS, IEC 62083 / 60601 compliance, commissioning per AAPM TG-53, and clinical physics QA — none of which is in scope here. The UI banner on every `/plan` page makes this unmistakable; the `dose_summary` JSONB payload carries the disclaimer string in code; OAR constraints come from published RTOG/QUANTEC values without institutional adaptation. |
